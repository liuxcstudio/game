...
(function () {
    ...
    function tryPatch() {
        if (patchGUIPackage()) {
            console.log('[FairyGUI] Patch applied after ' + _attempts + ' attempts');
            return;
        }
        if (++_attempts < 100) setTimeout(tryPatch, 100); // 减少检测频率，避免干扰加载
    }

    if (typeof cc !== 'undefined') tryPatch();
    else if (typeof document !== 'undefined') document.addEventListener('DOMContentLoaded', function () { setTimeout(tryPatch, 200); });
})();
